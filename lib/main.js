...
for(i = 0; i < numOfElem; i++){
    ...
    switch(a){
        ...
        case 1:
            ...
            break;
        case 2:
            elem = HTMLelements.containers[Math.floor(Math.random() * HTMLelements.containers.length)];
            elemPlace = document.createElement(elem);
            break;
        case 3:
            elem = HTMLelements.media[Math.floor(Math.random() * HTMLelements.media.length)];
            elemPlace = document.createElement(elem);
            const srcTxtArr = []
            for(l = 0; l < Math.floor(Math.random() * 40); l++){
                srcTxtArr.push(symbols.allSymbols[Math.floor(Math.random() * symbols.allSymbols.length)]);
            }
            srcTxtArr.push(mediaExtensions[Math.floor(Math.random() * mediaExtensions.length)]);
            srcTxtArr.unshift("https://");
            elemPlace.src = (srcTxtArr.join(""));
        case 4:
            elemPlace = document.createElement("input");
            const a = inputTypes[Math.floor(Math.random() * inputTypes.length)];
            elemPlace.setAttribute("type", a);
            break;
    }
